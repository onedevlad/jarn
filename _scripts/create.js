...
selection_box.rendering[1]=function(event){
	event.preventDefault();
	if( event.clientX >= id('canvas').getBoundingClientRect().right  ||
	    event.clientX <= id('canvas').getBoundingClientRect().left   ||
	    event.clientY >= id('canvas').getBoundingClientRect().bottom ||
	    event.clientY <= id('canvas').getBoundingClientRect().top    ){
		return;
	}
	...
	if(Math.max(event.clientX, selection_box.start_x) == event.clientX){										 // → From left to right
		id('selection_box').style.left=selection_box.start_x+'px';
		id('selection_box').style.right=(document.documentElement.clientWidth-selection_box.start_x+w)+'px';
	}
	else{																															 // ← From right to left
		id('selection_box').style.right=(document.documentElement.clientWidth-selection_box.start_x)+'px';
		id('selection_box').style.left=(selection_box.start_x-w)+'px';
	}
	if(Math.max(event.clientY, selection_box.start_y) == event.clientY){										 // ↓ From top to bottom
		id('selection_box').style.top=selection_box.start_y+'px';
		id('selection_box').style.bottom=(document.documentElement.clientHeight-selection_box.start_y+h)+'px';
	}
	else{																															 // ↑ From bottom to top
		id('selection_box').style.bottom=(document.documentElement.clientHeight-selection_box.start_y)+'px';
		id('selection_box').style.top=(selection_box.start_y-h)+'px';
	}
	id('selection_box').style.width=w+'px';
	id('selection_box').style.height=h+'px';

	evt.set([id('selection_box'), id('canvas')], 'mouseup', 'canvas.define_mouse_behavior(event, 2)');
